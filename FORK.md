...
## GraalVM platform metadata

The fork keeps Skiko as a platform library next to the packaged executable and
prevents the same native binary from being embedded a second time in the
GraalVM image. Applications can also exclude metadata for implementations that
cannot run on the current target:

```kotlin
graalvm {
    metadataExcludedTypePrefixes.addAll(
        "com.example.player.linux",
        "com.example.player.windows",
    )
}
```

Matching is package/class-boundary aware and affects only Nucleus-generated
reflection and JNI metadata; it does not remove bytecode from the runtime
classpath. Keep exclusions target-specific because removing required metadata
can cause native-image runtime failures.
